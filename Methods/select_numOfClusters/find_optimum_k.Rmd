...
This script will execute the following processes:    
...
536 studies). We want to create a synthetic dataset as a negative control with 
the distance ranging median or higher.
...
`neg_controls.R` script above. Distance ranges roughly between 1st quarter and 
3rd quarter of training datasets' distance distribution. 

```{r collapse=FALSE, echo=FALSE, eval=TRUE}
dat_dir <- "~/data2/GenomicSuperSignatureLibrary/refinebioRseq/Neg_Controls"
...
We plot the heatmap of distance matrix with different number of negative 
controls (PC1s from synthetic datasets) and confirmed that they can serve 
...
Next we combined top 20 PCs of training datasets and PC1s from varying numbers 
of negative controls (10, 20, 30, 40, and 50) and performed hierarchical 
clustering with different numbers of clusters. Here, we plotted the proportion 
of separated negative controls against the number of clusters.
...
## Proportion of the separated negative controls
This plot shows the proportion of the negative controls that are separated 
under different numbers of clusters. For all five different numbers of negative
controls, more than 90% of them were separated when **d = 4** and 100% of them 
were separated when **d = 2.25**.

```{r plot1, echo=FALSE, fig.width=13, fig.height=8}
...
```{r echo=FALSE, eval=TRUE, fig.width=13, fig.height=8}
knitr::include_graphics("outputs/png/B.separatedPortion.png")
```

## Number of the non-separated negative controls
...
```{r echo=FALSE, eval=TRUE, fig.width=6.5, fig.height=4}
knitr::include_graphics("outputs/png/C.numUnseparated.png")
```

```{r sup_fig_3, fig.width=18, fig.height=4.5, echo=FALSE, eval=FALSE}
