...
`studyMeta.tsv` table is available through `GenomicSuperSignature` package. In this
table, we summarized the details on GenomicSuperSignature models and their training
...
# Compare to multiPLIER training datasets
...
a <- length(unique(recount2$sample))
b <- length(unique(recount2$study))

## GenomicSupersignature training data
c <- length(unique(res_all$sample_accession))
d <- length(unique(res_all$study))

## Overlapping training data
e <- length(intersect(unique(recount2$sample), unique(res_all$sample_accession)))
f <- length(intersect(unique(recount2$study), unique(res_all$study)))
```

1. multiPLIER used `r a` unique runs from `r b` studies.   
2. GenomicSuperSignature used `r c` unique runs from `r d` studies.   
3. There are `r e` overlapping runs from `r f` studies used for both models.
...
## Save summary table
write.table(studyMeta, "~/data2/GenomicSuperSignature/inst/extdata/studyMeta.tsv")
