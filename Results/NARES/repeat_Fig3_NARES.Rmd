...
- Expression cata is downloaded from [here](https://github.com/greenelab/multi-plier/blob/master/data/expression_data/NARES_SCANfast_ComBat_with_GeneSymbol.pcl)      
...
As in multiPLIER paper, we tested if *neutrophil-associated* PCclusters from PCAmodel 
...
## Only with the top 5 PCclusters
...
for (i in seq_along(validated_ind)) {
  plot <- LVScatter(dat, paste0("PCcluster", validated_ind[i]), 
                    y.var = "MCP_estimate",
...
for (i in seq_along(sig_ind)) {
  plot <- LVScatter(dat2, paste0("PCcluster", sig_ind[i]), 
                    y.var = "MCP_estimate",
                    ylab = "MCPcounter neutrophil estimate")
  plots[[i]] <- plot
}
...
```{r save_for_paper, fig.width=4, fig.height=4, echo=FALSE}
## This chunk was executed only to save the summary table, not for vignettes.
plot <- LVScatter(dat2, paste0("PCcluster", 1551), 
                  y.var = "MCP_estimate",
                  ylab = "MCPcounter Neutrophil Estimate")
saveRDS(plot, "nares_neutrophil.rds")
```
