...
for (var in vars) {
  ...
  for(set in setNames) {
    ...
    if(length(count.table) <= 1) {
      ...
      model.all <- tryCatch(glm((outcome == "TRUE") ~ factor(cms_label_SSP) + pData.tmp[,m2_1] + pData.tmp[,m2_2],
...
df.toplot$Model[df.toplot$Model == 'LR.disc'] <- "CMS"
df.toplot$Model <- factor(df.toplot$Model, levels = c("CMS", m2_name))
