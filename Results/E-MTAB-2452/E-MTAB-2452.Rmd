...
This expression data is pre-processed by Greene lab and available [here](https://github.com/greenelab/multi-plier/blob/master/data/expression_data/E-MTAB-2452_hugene11st_SCANfast_with_GeneSymbol.pcl).
...
analyzed in multiPLIER paper, we used the model annotated with PLIER priors.
...
# Plot
## Annotate top PCs
We checked the enriched pathways for top 8 PCs of E-MTAB-2452 data. Under the
default, RAVs with the validation score above 0.5 are assigned. Here, only PC1
and PC2 have associated RAV under the default condition.
```{r}
annotatePC(1:8, val_all, RAVmodel, n = 5, simplify = TRUE)  
```

We lowered the validation score cutoff to 0 and the output returned with more 
enriched pathways.
```{r}
...
## Pairs plot
Different pairs of top 4 PCs are plotted in pairs plot.
```{r echo=FALSE}
library(tibble)
